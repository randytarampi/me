...
class Post {
	constructor(id, type, source, dateCreated, datePublished, title, body, sourceUrl, creator) {
		...
		this.source = source;
		this.datePublished = datePublished && Moment.utc(datePublished);
		this.dateCreated = dateCreated && Moment.utc(dateCreated) || this.datePublished && this.datePublished.clone();
		this.title = title;
		this.body = body;
