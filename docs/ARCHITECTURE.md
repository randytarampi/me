...
- The build is ESM-only end-to-end: every workspace package's `main` field points at its `esm/` library output (built via `build:babel:esm`), and `www`/`letter`/`resume`'s browser bundles are built via a single `webpack.client.config.esm.js`/`client.esm.webpack` path with no `nomodule`/ES5 fallback. The legacy ES5 web build (`webpack.client.config.es5.js`, the `client.es5` Babel env, and `layout.pug`'s differential `nomodule`/`module` script tags) was dropped entirely in a later session, once it was confirmed no `browserslist` config anywhere in the repo justified keeping it.
...
- `packages/www/test/integration/dist/browserBundle.js` is a headless-browser smoke test (Puppeteer) that boots a local static server for `packages/www/dist`, loads the built ESM bundle in a real Chromium instance, and asserts zero uncaught runtime errors plus real rendered content - closing the gap where the last several `www` dev-server regressions (ESM/CommonJS module-wrapper mismatches, `immutable` default-import interop) could only ever be caught by a human opening a real browser. It originally used `jsdom`, relying on it skipping `<script type="module">` tags and falling back to a `nomodule`/ES5 sibling bundle; once the ES5 web build was dropped, that fallback no longer existed, so it was rewritten to use Puppeteer, which actually executes `type="module"` scripts.
