...
- **NODE_ENV/NODE_CONFIG_ENV decoupling**: Build scripts inherit `NODE_ENV` and `NODE_CONFIG_ENV` from the caller. When neither is set, `babel.config.js` defaults `NODE_CONFIG_ENV` to `prd` for config-file resolution; explicit values remain untouched. See `util.cjs` and `docs/ARCHITECTURE.md`'s "Build pipeline" section.
...
- **Show, don't deploy.** Before any dev push of user-visible changes, run the local lifecycle and either drive it headlessly for DOM/geometry evidence or hand the user `http://localhost:8080` to eyeball. Dev pushes verify deployment; they are not the review surface.
