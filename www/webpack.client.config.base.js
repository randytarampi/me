...
const crispWebsiteId = String(config.get("crisp.app.id") || "").trim();
...
const buildViewForPageUrl = (pageName, pageUrl = config.get("www.publishUrl")) => {
    ...
    return new HtmlWebpackPlugin({
        ...
        templateParameters: buildPugLocals({
            ...
            packageJson,
            pageUrl,
            injectedScript: crispWebsiteId
                ? `<script>window.$crisp=[];window.CRISP_WEBSITE_ID="${crispWebsiteId}";(function(){d=document;s=d.createElement("script");s.src="https://client.crisp.chat/l.js";s.async=1;d.getElementsByTagName("head")[0].appendChild(s);})();</script>`
                : "",
            injectedNoScript: [
                `<noscript>
