...
const plugins = [
    new webpack.DefinePlugin({
        ...
        __LETTER_APP_URL__: JSON.stringify(config.get("letterAppUrl")),
        __ASSET_URL__: JSON.stringify(config.get("assetUrl")),
        __PUBLISHED_RESUME_URL__: JSON.stringify(config.get("resume.publishUrl")),
        __PUBLISHED_LETTER_URL__: JSON.stringify(config.get("letter.publishUrl"))
    })
];
...
module.exports = {
    ...
    devtool: "source-map",
    entry: {
        letter: ["@babel/polyfill", `${__dirname}/public/views/index.jsx`]
    },
    output: {
