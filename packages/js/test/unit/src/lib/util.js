...
describe("util", function () {
    ...
    describe("firstResolved", function () {
        it("resolves the first of the resolved", function () {
            ...
            return util.firstResolved([
                    Promise.resolve(first),
                    util.timedPromise(last)
                ])
                .catch(error => {
        ...
        it("resolves the resolved", function () {
            ...
            return util.firstResolved([
                    util.timedPromise(null, new Error(`${first} exploded`), true),
                    util.timedPromise(last)
                ])
                .catch(error => {
                    expect(error).to.not.be.ok;
        ...
        it("throws if none resolved", function () {
            ...
            return util.firstResolved([
                    util.timedPromise(null, new Error(`${first} exploded`), true),
                    util.timedPromise(null, new Error(`${last} exploded`), true)
                ])
                .then(() => {
                    throw new Error("Wtf? This should've thrown");
                })
